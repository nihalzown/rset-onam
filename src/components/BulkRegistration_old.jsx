...
const BulkRegistration = () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .then(({ data, error }) => {
          ...
          // Count registrations per house
          const counts = {};
          HOUSES.forEach(house => counts[house] = 0);
          
          data?.forEach(registration => {
            counts[registration.house]++;
          });
  ...
  const validateForm = () => {
    ...
    if (!selectedHouse) {
      newErrors.push('Please select a house');
    }

    // Check if house already has 30 registrations
    if (housesStatus[selectedHouse] >= 30) {
      newErrors.push(`${selectedHouse} has already completed their registration`);
    }

    // Check if all fields are filled
    team.forEach((participant, index) => {
      if (!participant.name.trim()) {
        newErrors.push(`Participant ${index + 1}: Name is required`);
      }
      if (!participant.college_id.trim()) {
        newErrors.push(`Participant ${index + 1}: College ID is required`);
      }
    });

    // Check for duplicate college IDs within the form
    const collegeIds = team.map(p => p.college_id.trim()).filter(id => id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Prepare data with house information
      const teamWithHouse = team.map(participant => ({
        ...participant,
        house: selectedHouse,
        name: participant.name.trim(),
        college_id: participant.college_id.trim()
      }));
      ...
      alert(`Successfully registered ${teamWithHouse.length} participants for ${selectedHouse}!`);
